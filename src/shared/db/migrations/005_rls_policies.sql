...
ALTER TABLE public.alarm_streak_events ENABLE ROW LEVEL SECURITY;
...
DROP POLICY IF EXISTS "Users can read their own streak events" ON public.alarm_streak_events;
CREATE POLICY "Users can read their own streak events"
ON public.alarm_streak_events
FOR SELECT
TO authenticated
USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can insert their own streak events" ON public.alarm_streak_events;
CREATE POLICY "Users can insert their own streak events"
ON public.alarm_streak_events
FOR INSERT
TO authenticated
WITH CHECK (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update their own streak events" ON public.alarm_streak_events;
CREATE POLICY "Users can update their own streak events"
ON public.alarm_streak_events
FOR UPDATE
TO authenticated
USING (auth.uid() = user_id)
...
DROP POLICY IF EXISTS "Users can delete their own streak events" ON public.alarm_streak_events;
CREATE POLICY "Users can delete their own streak events"
ON public.alarm_streak_events
FOR DELETE
TO authenticated
USING (auth.uid() = user_id);
